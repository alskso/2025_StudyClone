...
function StoreMain({ location, setLocation, setShowModal, products }: StoreMainProps) {
  // ✅ 위치 필터 state
  ...
  return (
    <>
      ...
      <S.SearchContainer>
        <SearchBar location={location} setLocation={setLocation} setShowModal={setShowModal} products={products}></SearchBar>
      </S.SearchContainer>
      <S.ContentContainer>
        ...
        {/* 페이지 설명 */}
          <S.PageNav> 
            홈 &gt; <b>중고거래</b> 
            {/* 주소데이터로 변경 필요! */}
            <S.PositionTitle>서울특별시 도봉구{""} <span>{selectedLocation ? selectedLocation : "전체"}</span> 중고거래</S.PositionTitle>
          </S.PageNav>
        ...
        <S.SeperationBox>
          <S.SideFilter>
            ...
            <S.FilterBox>
              <b>가격</b>
              <S.PriceFilterButton>
                <input type="radio" name="price" value="0" checked={selectedPrice === 0} onChange={(e) => setSelectedPrice(Number(e.target.value))} />
                <span>나눔</span>
              </S.PriceFilterButton>
              <S.PriceFilterButton>
                <input type="radio" name="price" value="5000" checked={selectedPrice === 5000} onChange={(e) => setSelectedPrice(Number(e.target.value))}/>
                <span>5,000원 이하</span>
              </S.PriceFilterButton>
              <S.PriceFilterButton>
                <input type="radio" name="price" value="10000" checked={selectedPrice === 10000} onChange={(e) => setSelectedPrice(Number(e.target.value))}/>
                <span>10,000원 이하</span>
              </S.PriceFilterButton>
              <S.PriceFilterButton>
                <input type="radio" name="price" value="20000" checked={selectedPrice === 20000} onChange={(e) => setSelectedPrice(Number(e.target.value))}/>
